...
def reporte_sueldos():
    nombre_archivo = "report_sueldos.csv"
    with open(nombre_archivo,mode="w",newline=('')) as archivo_csv:
        escritor_csv = csv.writer(archivo_csv, delimiter=(','))
        escritor_csv.writerow(['Trabajador',  'Sueldo',  'Descuento AFP', 'Descuento Salud', 'Sueldo Neto'])

        for trabajador, sueldo in sueldos.items():
            descuento_salud = int(sueldo * 0.07)
            descuento_afp =  int(sueldo * 0.012)
            sueldo_liquido = sueldo-descuento_afp - descuento_salud

            escritor_csv.writerow([trabajador,sueldo,descuento_afp,descuento_salud,sueldo_liquido])
            
    print(f"El archivo {nombre_archivo} se ha generado exitosamente")
